    ...
    (unwind-protect
        (progn
          ...
          (cl-letf (((symbol-function 'completing-read)
            ...
            (should sync-called)
            (should (string-match-p (regexp-quote "Scope: all visible buffers in current window.") captured-prompt))
    ...
    (unwind-protect
        (progn
          (cl-letf (((symbol-function 'completing-read)
            ...
            (ai-code-create-notes)
            (should (file-exists-p expected-file))
            (should (string-match-p (regexp-quote "Scope: current git repository at /tmp/repo.") captured-prompt))
            (should (string-match-p (regexp-quote (format "Target note file: %s" expected-file)) captured-prompt))
