...
(ert-deftest ai-code-test-git-worktree-branch-creates-symlink-for-files-dir ()
  "After worktree creation, symlink ai-code-files-dir-name to original repo."
  (let* ((temp-worktree-root (make-temp-file "ai-code-worktree-root-" t))
         (temp-git-root (make-temp-file "ai-code-git-root-" t))
         (ai-code-git-worktree-root temp-worktree-root)
         (branch "feature/symlink-test")
         (start-point "main")
         (original-files-dir (expand-file-name ai-code-files-dir-name temp-git-root))
         (repo-dir (expand-file-name
                    (file-name-nondirectory (directory-file-name temp-git-root))
                    temp-worktree-root))
         (worktree-path (expand-file-name branch repo-dir))
         (expected-link (expand-file-name ai-code-files-dir-name worktree-path)))
    (unwind-protect
        (progn
          ;; Create the original .ai.code.files directory
          (make-directory original-files-dir t)
          ;; magit-call-git returns 0 (success) and creates the worktree dir
          (cl-letf (((symbol-function 'ai-code--validate-git-repository)
                    ...
                    ((symbol-function 'magit-diff-visit-directory)
                     (lambda (_path) nil)))
            (ai-code-git-worktree-branch branch start-point)
            ;; Symlink should exist and point to original files dir
            (should (file-symlink-p expected-link))
            (should (string= (file-truename expected-link)
                             (file-truename original-files-dir)))))
      (delete-directory temp-worktree-root t)
      (delete-directory temp-git-root t))))

(ert-deftest ai-code-test-git-worktree-branch-skips-symlink-when-no-files-dir ()
  "Skip symlink creation when original ai-code-files-dir-name does not exist."
  (let* ((temp-worktree-root (make-temp-file "ai-code-worktree-root-" t))
         (temp-git-root (make-temp-file "ai-code-git-root-" t))
         (ai-code-git-worktree-root temp-worktree-root)
         (branch "feature/no-files-dir")
         (start-point "main")
         (repo-dir (expand-file-name
                    (file-name-nondirectory (directory-file-name temp-git-root))
                    temp-worktree-root))
    ...
    (unwind-protect
        (progn
          ;; Do NOT create original .ai.code.files directory
          (cl-letf (((symbol-function 'ai-code--validate-git-repository)
                     (lambda () temp-git-root))
                    ((symbol-function 'magit-call-git)
                     (lambda (&rest _args)
            ...
            (should-not (file-exists-p expected-link))))
      (delete-directory temp-worktree-root t)
      (delete-directory temp-git-root t))))
