...
See [[https://github.com/tninja/ai-code-interface.el/tree/main#ai-coding-cli-backend][AI coding CLI backend]] for the full backend list and configuration details.
...
falls back to Ghostel. See [[./docs/windows.org][Native Windows Support]] for setup and troubleshooting.
...
- `cape` / `company`: For inline completion of prompts you have written before, described under [[*Reusing Prompts You Already Wrote][Reusing Prompts You Already Wrote]]. Prompt completion itself needs neither package. `cape`, when installed, adds English dictionary words to the same candidate list with no configuration; `company` shows the candidates as you type once you opt in. Neither is a dependency, and neither is loaded when it is not installed.
...
- *Seamless Prompt Management*: Open the prompt file via `ai-code-open-prompt-file` (stored under `.ai.code.files/.ai.code.prompt.org` inside a Git repository), send regions with `ai-code-prompt-send-block`, and reuse prompt snippets via `yasnippet` to keep conversations organized. If the preferred prompt-history location is not writable, AI Code falls back to `~/.ai.code.files/.ai.code.prompt.org`; customize `ai-code-prompt-fallback-directory` to change that location or set it to nil to disable fallback. Prompt sending still proceeds with a warning when prompt history cannot be written.
  - Prompts you typed by hand can be completed back inline with `ai-code-prompt-completion-capf`, indexed across every repository. See [[*Reusing Prompts You Already Wrote][Reusing Prompts You Already Wrote]].
...
*** Reusing Prompts You Already Wrote

To show Company suggestions while typing directly into an AI Code =vterm=
session (including Claude Code and Codex), install =company= and set:

#+begin_src emacs-lisp
(setq ai-code-backends-infra-enable-terminal-completion t)
#+end_src

This works across native CLI agents using the same terminal infrastructure.
For an existing session, run =M-x ai-code-terminal-completion-mode=.  After
three letters, Company offers single-line prompt history and, if Cape is
installed, dictionary words.  =TAB= accepts the selected candidate without
submitting it; =C-n= and =C-p= choose a candidate, =ESC= dismisses, and =RET=
still submits to the CLI.  The feature is off by default and currently
requires =vterm=.  It tracks ordinary typing and backspace at the terminal
cursor.  Cursor moves, pasted text, CLI history and native CLI completions
discard the tracked suffix; keep typing to start a new completion.  It cannot
reconstruct input edited by the CLI itself.  Multi-line history entries are
excluded from terminal suggestions.

Most of what you retype into a prompt is wording, not structure: "explain the current code", the handoff instructions you send at every backend switch. This completes those from the prompt history files AI Code already keeps, across every repository it knows about -- projectile's known projects, `ai-code-prompt-completion-roots`, the current directory and the fallback location, with remote (Tramp) roots skipped so completion never opens a connection. Point `ai-code-prompt-completion-files` (nil by default) at files of your own -- a prompt library you keep by hand, a diary, any Org file whose wording you reuse. Setting `ai-code-prompt-completion-use-org-roam` adds the notes org-roam tracks the same way -- off by default, since a whole zettelkasten contributes note sections rather than prompts. In both, every headline offers two prompts however deep it sits, its own text and the text written under it, and whatever stands before the first headline -- the whole of a file written without any -- is split on blank lines. Datetree headlines, tags, statistics cookies and link wrappers are markup rather than wording and drop out, as does a heading of a word or two, quicker typed than picked from a popup. Neither history filter below applies there: you wrote those files, so they are offered as written. A large diary is worth measuring before you point at it -- 10MB of Org indexes in about a second and contributes on the order of 10^5 candidates.

It needs no configuration: opening a prompt file installs the capf, so `completion-at-point` (=M-TAB=) offers your earlier prompts right away, and if =cape= is installed its `cape-dict` words join the same candidate list. Only the popup front-end is left to you:

#+begin_src emacs-lisp
;; Show candidates as you type, using company
(add-hook 'ai-code-prompt-mode-hook #'ai-code-prompt-completion-setup)
#+end_src

Both =cape= and =company= are optional: neither is a dependency, and neither is loaded when it is not installed. Set `ai-code-prompt-completion-enable-company` to nil when a front-end such as corfu shows the candidates instead, or `(remove-hook 'ai-code-prompt-mode-hook #'ai-code-prompt-completion--enable)` to opt out completely. If your own config already merges capfs with cape, add `ai-code-prompt-completion-capf` to that list rather than relying on the default: the first capf at a given hook depth answers, and yours is added first.

Candidates are the prompts you wrote by hand, ranked by how often you wrote them, shown as a truncated first line and inserted whole. A prompt is recalled from any word of three characters or more in its first line, not only the opening one, and choosing it rewrites the line from the start when what you already typed there opens that same prompt -- so a half-finished `- Go ahead with the suggested refac` completes in place, while a `TODO: ` of your own stays put. Matching still begins at a word: Emacs only offers a non-exclusive capf once plain prefix completion could succeed, so `factor` reaches `refactoring` only if your own `completion-styles` include `flex`. Prompts AI Code generated itself are left out by prefix (`ai-code-prompt-completion-template-prefixes`), and anything longer than `ai-code-prompt-completion-max-lines` is dropped as a one-off paste. The index is cached for the session; `ai-code-prompt-completion-refresh` rebuilds it.

One caveat on the dictionary half: `cape-dict` reads `/usr/share/dict/words`, which on macOS is Webster's 1934 edition. For a modern list, generate one and point `cape-dict-file` at it:
...
[[file:./symbol-completion-example-screenshot.png]]
...
- `ai-code-grill-me-enabled`: when non-nil, offer a one-shot clarification step (/"Grill me before acting?"/) immediately before the completed prompt is sent for `ai-code-code-change`, `ai-code-ask-question`, `ai-code-implement-todo`, and `ai-code-send-command`.  When accepted, the AI reads the bundled =prompt/grilling.v1.md= harness and asks targeted clarifying questions before acting, reducing ambiguity in the request.  The harness is adapted from [[https://github.com/mattpocock/skills/blob/main/skills/productivity/grilling/SKILL.md][mattpocock/skills – grilling]].  Disabled by default; enable it with:
  ...
  #+end_src
- `ai-code-use-compose-buffer`: when non-nil, edit prompts longer than two lines in a full =*AI Compose*= buffer instead of the minibuffer. Accept with =C-c C-c=, cancel with =C-c C-k=; =@= file completion works as in prompt buffers. Every invocation starts from a fresh buffer and any stale compose buffer is discarded. Disabled by default; enable it with:
  #+begin_src emacs-lisp
  (setq ai-code-use-compose-buffer t)
  #+end_src
...
**** Open Interpreter CLI setup
     ...
     How to setup [[https://www.openinterpreter.com/docs/terminal/kimi-k3?utm_source=github&utm_medium=referral&utm_campaign=readme&utm_content=kimi_k3_docs][Kimi K3 for Open Interpreter CLI]]
