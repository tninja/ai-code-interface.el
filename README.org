...
- Prompt file name customization: set `ai-code-prompt-file-name` to change the default prompt file name created under `.ai.code.files/`.
- Prompt path preprocessing: when `ai-code-prompt-preprocess-filepaths` is non-nil, file paths in prompts are normalized to `@relative/path` within the current repo.
