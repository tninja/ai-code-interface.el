...
Set the backend to =codex= or =github-copilot-cli= and start a session with =C-c a a=. AI Code will:
...
Today this auto-wiring is enabled for =codex= and =github-copilot-cli=. The adapter layer is designed so other backends can be added later.

If you disabled it before, enable it again with:

#+begin_src emacs-lisp
(setq ai-code-mcp-agent-enabled-backends '(codex github-copilot-cli))
#+end_src

***** Make Sure It Works

1. Start =codex= or =github-copilot-cli= with =C-c a a= from the file or project you want to work on.
...
# DONE: how do we add a new emacs mcp function and make it available inside ai coding
#   agent, just like other mcp function you showed on README? update here
...
3. Start =codex= or =github-copilot-cli= with =C-c a a=.
