...
- `cape` / `company`: For inline completion of prompts you have written before, described under [[*Reusing Prompts You Already Wrote][Reusing Prompts You Already Wrote]]. `cape` adds English word completion to the same popup and `company` displays it. Neither is required by AI Code, neither is loaded unless you opt in with `ai-code-prompt-completion-setup`, and prompt completion still works through the built-in `completion-at-point` (`M-TAB`) with neither installed.
...
AI Code never registers the capf behind your back; your completion setup stays yours. One line turns on the whole thing:

#+begin_src emacs-lisp
(add-hook 'ai-code-prompt-mode-hook #'ai-code-prompt-completion-setup)
#+end_src

That installs `ai-code-prompt-completion-dict-capf`, which offers prompts you wrote before and, when =cape= is installed, English words from `cape-dict` in the same popup. `company-mode` displays them; set `ai-code-prompt-completion-enable-company` to nil if another front-end such as corfu does that job, or if you would rather call `completion-at-point` (=M-TAB=) by hand.

Both =cape= and =company= are optional. Neither is a dependency of AI Code, neither is loaded until the setup function runs, and each is skipped when it is not installed -- with neither one present you still get prompt completion through the built-in `completion-at-point`.

If you already merge capfs with cape, skip the setup function and add the capf to your own list instead. Only the first capf at a given hook depth gets to answer, so a second entry would just race yours:
