...
- *Changing Code*: Position the cursor on a function or select a region of code. Press `C-c a`, then `c` (`ai-code-code-change`). Describe the change you want to make in the prompt. The AI will receive the context of the function or region and your instruction. When ai-code-test-after-code-change is t, AI will run test after code changes and follow up on results.
- *Implementing a TODO*: Write a comment in your code, like `;; TODO: Implement caching for this function`. Place your cursor on that line and press `C-c a`, then `i` (`ai-code-implement-todo`). The AI will generate the implementation based on the comment. When ai-code-test-after-code-change is t, AI will run test after code changes and follow up on results.
...
- `ai-code-test-after-code-change`: Selects how prompts request tests after code changes (suffix, TDD Red+Green, or off).
- `ai-code-test-after-code-change-user-suffix`: Optional custom suffix used when `ai-code-test-after-code-change` is set to `suffix` (falls back to `ai-code-test-after-code-change-suffix` if nil).
