...
Registered tools are stored in =ai-code-mcp-server-tools=.
...
***** Dispatch MCP Methods from Elisp

You can inspect registered tools:

#+begin_src emacs-lisp
(ai-code-mcp-dispatch "initialize")
(ai-code-mcp-dispatch "tools/list")
#+end_src

You can also call a tool directly:
...
***** Use Session-Aware Project Context

If a tool should run with a specific project root and active buffer, register a session first:
...
For lower-level integrations, =ai-code-mcp-with-session-context= executes arbitrary Lisp forms with the registered session buffer and project directory.
