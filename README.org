...
#+begin_src emacs-lisp
  (use-package ai-code
    ...
    ;; Optional: Enable desktop notifications when AI responses are ready
    ;; (setq ai-code-notifications-enabled t) ;; enabled by default
...
- When the terminal has been idle for ~1.5 seconds (configurable via `ai-code-backends-infra-idle-delay`), it's considered a completed response.
...
**** Customization
You can customize notification behavior with these variables:
#+begin_src emacs-lisp
  ;; Enable/disable notifications
  (setq ai-code-notifications-enabled t)
  
  ;; Show notifications when AI completes a response
  ...
  ;; Idle delay before considering response complete (in seconds)
  (setq ai-code-backends-infra-idle-delay 1.5)
#+end_src
