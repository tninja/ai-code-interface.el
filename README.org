...
Candidates are the prompts you wrote by hand, ranked by how often you wrote them, shown as a truncated first line and inserted whole. Prompts AI Code generated itself are left out by prefix (`ai-code-prompt-completion-template-prefixes`), and anything longer than `ai-code-prompt-completion-max-lines` is dropped as a one-off paste. Recall is by prefix -- a `flex` style cannot help here, because Emacs only offers a non-exclusive capf once plain prefix completion could succeed. The index is cached for the session; `ai-code-prompt-completion-refresh` rebuilds it.
