...
*** Backend Configuration
   ...
   - Press `C-c a` then `S` (`ai-code-install-backend-skills`) to install backend skills for the currently selected backend.
     ...
     - Those skills pair well with this package's existing harness-oriented workflows, especially the `ai-code-auto-test-type` feedback loop, `ai-code-tdd-cycle`, and `ai-code-pull-or-review-diff-file` code review flow.
...
**** ECA (Editor Code Assistant) backend setup
     ...
     ai-code-menu integration, workspace management, auto project/session syncing,
     shared context, and programmatic context management (file, clipboard, cursor, repo-map):
     #+begin_src emacs-lisp
     (require 'eca-ext)
     #+end_src
