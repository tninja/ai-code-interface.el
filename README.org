...
Set the backend to =codex= and start a session with =C-c a a=. AI Code will:
- start a local Emacs MCP HTTP server
- register the current project/buffer as the MCP session context
- inject a session-scoped MCP config into that Codex session automatically

Today this auto-wiring is enabled for =codex=. The adapter layer is designed so other backends can be added later.

If you disabled it before, enable it again with:

#+begin_src emacs-lisp
(setq ai-code-mcp-agent-enabled-backends '(codex))
#+end_src

***** Make Sure It Works

1. Start Codex with =C-c a a= from the file or project you want to work on.
2. Switch to the Codex buffer and run:

#+begin_src emacs-lisp
(ai-code-mcp-agent-buffer-status)
#+end_src

If it works, you will get a plist with =:backend codex=, a =:session-id=, and a local =:server-url=.

Then ask Codex to use one of the Emacs tools, for example:
- use =project_info= and tell me the current project info
- use =imenu_list_symbols= for the current file

If Codex can answer with your current project path, buffer context, or symbols from the current file, then the active agent session is using the Emacs MCP tools.
