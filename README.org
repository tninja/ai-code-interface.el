...
*** Pasting Large Text Blocks
    When you need to paste large blocks of text (such as error messages or code snippets) into an AI session:
    
    - Copy the text to your clipboard or kill ring
    - Use `M-x ai-code-cli-paste-text` to paste it properly into the AI session
    - Or use the backend-specific commands:
      - `M-x codex-cli-paste-text` for Codex
      - `M-x github-copilot-cli-paste-text` for GitHub Copilot
    
    These commands handle large text blocks correctly by using the terminal's paste functionality, which avoids issues with bracketed paste mode in vterm.
