...
**** Built-in Emacs MCP Tools Core

This package now includes a transport-agnostic MCP tools core for exposing Emacs features to backend adapters and Lisp-side integrations. The current implementation focuses on the reusable server core, not on a standalone HTTP server process.

It currently provides:
- tool registration API
- MCP dispatch for =initialize=, =tools/list=, and =tools/call=
- session-aware project context
- built-in common Emacs tools:
  - =project_info=
  - =imenu_list_symbols=
  - =xref_find_references=
  - =treesit_info=

***** Enable the Built-in Tool Set
...
Check the registered tool names:
...
You can also check the MCP-facing view:

#+begin_src emacs-lisp
(ai-code-mcp-dispatch "tools/list")
#+end_src

If setup worked, the result contains those tool names under =tools=.

***** Register a Custom Tool

Use =ai-code-mcp-make-tool= to expose your own Emacs function:

#+begin_src emacs-lisp
(ai-code-mcp-make-tool
 :function (lambda (file_path)
             (with-current-buffer (find-file-noselect file_path t)
...
***** How Do I Know the Tools Actually Work?

Call a tool directly:

#+begin_src emacs-lisp
(let ((session-id "demo-session"))
  (ai-code-mcp-register-session session-id default-directory (current-buffer))
...
If this returns a =content= field whose text includes your project path, buffer name, and file count, then the tool is working inside Emacs.

You can test other tools the same way:

#+begin_src emacs-lisp
(ai-code-mcp-dispatch
 "tools/call"
 `((name . "imenu_list_symbols")
   (arguments . ((file_path . ,(buffer-file-name))))))
#+end_src

For lower-level integrations, =ai-code-mcp-with-session-context= runs arbitrary Lisp forms with the registered session buffer and project directory.

***** Can Codex CLI or Another AI Agent Use These Tools?

Not automatically yet.

What you can prove today:
- the tools are registered in Emacs
- the MCP core exposes them through =tools/list=
- the tools run correctly through =tools/call=

What you cannot prove from =ai-code-mcp-builtins-setup= alone:
- that Codex CLI is connected to this MCP core
- that Codex CLI has discovered these tools
- that Codex CLI can invoke them over a transport

Reason: the current implementation is only the MCP core inside Emacs. It does not yet start a standalone Streamable HTTP server or install a Codex-specific MCP adapter by itself.

So the practical rule is:
- if =tools/list= and =tools/call= work, the Emacs-side MCP core is ready
- an AI agent such as Codex CLI can use these tools only after a transport/adapter layer connects that agent to this core

***** Current Scope

This MCP implementation is intentionally small and composable:
- it provides the reusable MCP core inside Emacs
- it does not yet start a standalone Streamable HTTP listener on its own
- it is suitable for backend adapters, tests, and future transport layers
