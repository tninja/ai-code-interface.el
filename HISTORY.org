...
** 1.97
- Chore: Derive learning unit tests for a topic; make in-buffer prompt dump opt-in
- Feat: Architecture docs get topic scoping, navigable code links, and in-buffer prompts
- Chore: C-c a @: copy file + line range only for regions longer than 5 lines
- Feat: complete prompts you already wrote; fix input history data loss
- Feat: discuss the next valuable growth step for Org tasks
- Feat: choose which enclosing Org section scopes a prompt, and reference oversized sections by line range instead of inlining them
...
- feat: Add MCP support for Antigravity CLI, solution from lesteral
- Add optional compose buffer for long prompts
- Freeze scrolled-away vterm viewports during redraw
- Expand `C-c a w` into a worktree action menu: create, open directory, visit, delete
- Prevent MCP HTTP client buffer leaks
- Fix: C-c a w silently aborts at the new branch name prompt
...
- Improve native Windows support with Ghostel fallback

** 1.95
- Default PR target branch to the reflog-recorded parent branch
- Update Grill harness to frontier-based rounds
- Harden Uncle Bob harness (and +) against false-green checks
- Add commit current changes workflow
- Fix: adapt to ghostel 0.47+ link-detection API change
- Fix: retain cached session links outside each linkified tail across ghostel redraws
- Fix: keep ghostel plain-link detection asynchronous so session links survive redraws

** 1.94
- feat(treesit): symbol-level @# completion with qualified names
...
** 1.93
- Chore: sort backend selection by MRU order with history persistence
- Fix session lookup across symlinks and worktrees
- Fix prompt handling in read-only directories
...
- Feat: Add Uncle Bob's coding agent harness
- Tighten GPTel code-change prompt classification
- Harden and modernize Emacs MCP integration
- Simplify agile development menu actions
- Confirm prompt sending after starting a missing session
- Confirm before canceling a nonblank editor viewport and refocus its source, by cxa

** 1.91
- Add Grill support to some github related tool and scoped context copying, re-organize menu
- Fix: Make skill management prompts use global scope
- Feat: add Pi coding agent backend support, by cxa
...
** 1.900
- Feat: add open interpreter cli support, initial code from swithin chan, modified by ai coding agent
- Fix editor viewport status validation after temp directory changes
- Feat: Add native insertion into AI sessions
- Merge Grill support into the harness module; Resolve conflict between and follow up suggestion
- Refactor: Unify prompt suffix injection with ordered providers
- Fix editor viewport auto-submit timing, by cxa
- Feat: Add Emacs editor viewports for AI CLI sessions, by cxa
- Stabilize Ghostel image previews during scrolling and redraws, by fxa
- Feat: Add lightweight grill-me prompt flow
